...
class Library {
    ...
    searchByTitle(title) {
        for (let i = 0; i < this.shelves.length; i++) {
            if (this.shelves[i].searchByTitle(title) !== false) {
                let result = { 
                    shelf: i + 1,
                    index: this.shelves[i].searchByTitle(title).index,
                    book: this.shelves[i].searchByTitle(title).book,
                }
                return result;
            }  
        }
...
class User {
    ...
    borrowBook(title) {
        let borrowedBook = library.searchByTitle(title).book;
        this.booksBorrowed.add(borrowedBook);
        
        let name = this.name;

        library.searchByTitle(title).markAsIssued(name);
    }
}

const userOne = new User(1, 'User One');
userOne.borrowBook('Book One');
console.log(userOne);